...
export const ImportResume = () => {
  ...
  const extractDataFromText = (text) => {
    ...
    const phoneMatch = text.match(/(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}|\+\d{1,3}[-.\s]?\d{1,14}/);
    if (phoneMatch) {
    ...
    // Extract LinkedIn
    const linkedinMatch = text.match(/(?:linkedin\.com\/in\/|linkedin\.com\/pub\/|linkedin\.com\/profile\/view\?id=)([\w-]+)/i);
    if (linkedinMatch) {
    ...
    const websiteMatch = text.match(/(?:portfolio|website|site)[\s:]*([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})/i);
    if (websiteMatch) {
      extractedData.personalInfo.website = websiteMatch[1].includes('http') ? websiteMatch[1] : `https://${websiteMatch[1]}`;
    }

    // Extract address/location
    const locationMatch = text.match(/(?:address|location|based in|city)[\s:]*([^\n]{10,60})/i);
    if (locationMatch) {
    ...
    for (const pattern of summaryPatterns) {
      ...
      if (summaryMatch) {
        ...
        summary = summary
          .replace(/(SUMMARY|PROFILE|ABOUT|OBJECTIVE|PROFESSIONAL\s+SUMMARY)/gi, '')
          .trim()
          .split('\n')
          .map(line => line.trim())
          .filter(line => line && !line.match(/^[A-Z\s]+$/) && line.length > 10)
          .join(' ')
    ...
    for (const pattern of skillsPatterns) {
      ...
      if (skillsMatch) {
        const skillsText = skillsMatch[0].replace(/SKILLS|TECHNICAL\s+SKILLS|CORE\s+COMPETENCIES/i, '').trim();
        const skillsList = skillsText
          .split(/[,•\n|]/)
          .map(skill => skill.trim().replace(/[^\w\s+#.-]/g, ''))
          .filter(skill => skill && skill.length > 1 && skill.length < 40 && !skill.match(/^\d+$/))
          .slice(0, 15); // Limit to 15 skills

        if (skillsList.length > 0) {
          // Try to categorize skills
          const techSkills = skillsList.filter(skill =>
            /javascript|python|java|react|node|sql|html|css|php|c\+\+|c#|ruby|go|rust|swift|kotlin|typescript/i.test(skill) ||
            skill.length < 20
          );
          const tools = skillsList.filter(skill =>
            /git|docker|aws|azure|figma|photoshop|excel|jira|slack|trello/i.test(skill)
          );
    ...
    const experiencePattern = /(EXPERIENCE|WORK\s+HISTORY|EMPLOYMENT)[\s\S]*?(?=\n[A-Z\s]{4,}|\n\n|EDUCATION|SKILLS)/i;
    const experienceMatch = text.match(experiencePattern);
    if (experienceMatch) {
      const expText = experienceMatch[0].replace(/(EXPERIENCE|WORK\s+HISTORY|EMPLOYMENT)/i, '').trim();
    ...
    const educationPattern = /EDUCATION[\s\S]*?(?=\n[A-Z\s]{4,}|\n\n|SKILLS|EXPERIENCE)/i;
    ...
    try {
      ...
      if (fileType === 'text/plain') {
        ...
        try {
          ...
          throw new Error('Unable to parse Word document. Please try with a different file format.');
        }
      } else if (fileName.endsWith('.doc')) {
        // Legacy DOC format
        setStatusMessage('Legacy .doc files are not supported. Please convert to .docx, .pdf, or .txt format.');
      ...
      if (personalFields === 0 && !hasSkills && !hasProfile && !hasExperience && !hasEducation) {
        throw new Error('No recognizable resume data found. Please check your file format and content structure.');
      }
      ...
      const importedItems = [];
      if (personalFields > 0) importedItems.push(`${personalFields} personal detail${personalFields > 1 ? 's' : ''}`);
      if (hasProfile) importedItems.push('professional summary');
      if (hasSkills) importedItems.push(`${extractedData.skills.technical.length} skill${extractedData.skills.technical.length > 1 ? 's' : ''}`);
      if (hasExperience) importedItems.push(`${extractedData.experience.length} work experience${extractedData.experience.length > 1 ? 's' : ''}`);
      if (hasEducation) importedItems.push(`${extractedData.education.length} education record${extractedData.education.length > 1 ? 's' : ''}`);
  ...
  }, [updatePersonalInfo, updateProfile, updateSkills, addExperience, addEducation]);

  return (
    <div className="space-y-6">
      <div className="border-b border-gray-200 dark:border-gray-700 pb-3 mb-4">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">Import Resume</h2>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
        <h3 className="font-medium text-blue-900 dark:text-blue-300 mb-2">📎 Supported File Formats</h3>
        <div className="text-sm text-blue-700 dark:text-blue-300 space-y-2">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="flex items-center space-x-2">
              <span className="font-mono bg-blue-100 dark:bg-blue-800 px-2 py-1 rounded text-xs">.PDF</span>
              <span>Adobe PDF files</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="font-mono bg-blue-100 dark:bg-blue-800 px-2 py-1 rounded text-xs">.DOCX</span>
              <span>Microsoft Word</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="font-mono bg-blue-100 dark:bg-blue-800 px-2 py-1 rounded text-xs">.TXT</span>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg border border-amber-200 dark:border-amber-800">
        <h3 className="font-medium text-amber-900 dark:text-amber-300 mb-2">💡 Tips for Best Results</h3>
