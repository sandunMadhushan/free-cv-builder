...
export const ModernTemplate = ({ cvData, customization }) => {
      ...
      case "profile":
        return profile.summary ? (
          <div key="profile" className={spacingClasses.section}>
            <h2
              className={`${fontSizes.heading} ${accentClass} mb-3`}
              style={{ color: primaryColor }}
            >
              PROFESSIONAL SUMMARY
            </h2>
            <p
              className={`text-gray-700 dark:text-gray-200 leading-relaxed ${fontSizes.body}`}
            >
              {profile.summary}
            </p>
          </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {skills.technical.map((skill, index) => (
                      <span
                        key={index}
                        className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-2 py-1 text-sm rounded"
                      >
                        {skill}
                      </span>
                    ))}
                        ...
                        project.technologies.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {project.technologies.map((tech, index) => (
                              <span
                                key={index}
                                className="bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 px-2 py-0.5 text-xs rounded"
                              >
                                {tech}
                              </span>
                            ))}
  ...
  return (
    ...
    >
      ...
      <div
        className={`border-b-2 pb-6 ${spacingClasses.section}`}
        style={{ borderColor: primaryColor }}
      >
        <h1
          className={`${fontSizes.heading === "text-2xl" ? "text-4xl" : fontSizes.heading === "text-lg" ? "text-2xl" : "text-3xl"} ${accentClass} mb-2`}
          style={{ color: primaryColor }}
        >
        ...
        <div
          className={`flex flex-wrap gap-x-4 gap-y-1 ${fontSizes.small} text-gray-600 dark:text-gray-400`}
        >
          {personalInfo.email && (
