      ...
      (set, get) => ({
        ...
        renameVersion: (id, newName) => {
          set((state) => ({
            versions: state.versions.map((version) =>
              version.id === id
                ? { ...version, name: newName, updatedAt: new Date().toISOString() }
                : version
            ),
          }));
        },
        ...
        getSortedVersions: () => {
          const state = get();
          return [...state.versions].sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
        },
        ...
        duplicateVersion: (id) => {
          ...
          if (version) {
            const duplicatedVersion = createVersionSnapshot(version.data, `${version.name} (Copy)`);
        ...
        getStats: () => {
          ...
          return {
            ...
            oldestVersion: state.versions.length > 0
              ? Math.min(...state.versions.map(v => new Date(v.createdAt)))
              : null,
            newestVersion: state.versions.length > 0
